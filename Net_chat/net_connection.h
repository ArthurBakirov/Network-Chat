//
// Created by airwalk on 04-Jul-22.
//
...
namespace olc
{
    ...
    private:
        void ReadHeader(){
            asio::async_read(m_socket, asio::buffer(&m_msgBuff.header, sizeof(message_header<T>)),
        ...
        void ReadBody(){
            asio::async_read(m_socket, asio::buffer(m_msgBuff.body.data(), m_msgBuff.body.size()),
                    [this](std::error_code error, std::size_t bytes){
                if(!error) {
                    AddToIncomimgMessageQueue();
                } else {
       ...
       uint64_t Scramble(uint64_t input_val){
        ...
        void WriteValidation(){
            asio::async_write(m_socket, asio::buffer(&Validaton_code_out, sizeof(uint64_t)),
                              [this](const std::error_code& error, std::size_t bytes){
                                  if(!error) {
                                      if (m_nOwnerType == owner::client){
                                          ReadHeader();
        ...
        void ReadValidation(olc::net::server_interface<T>* server = nullptr){
            ...
            m_socket.async_read_some(asio::buffer(&Validaton_code_in, sizeof(uint64_t)),
